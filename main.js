...
=======
  ...
  console.log('🧹 Temp folder auto-cleaned');
>>>>>>> 573a56dafd42485427fefbc49a1b7ec41d71b3a4
}, 3 * 60 * 60 * 1000);
...
<<<<<<< HEAD
=======
const viewoncedmCommand = require('./commands/vvdm');
...
=======
            ...
            if (message.key.fromMe) {
                ...
                const pppronouns = ['his', 'her', 'their'];

    const ppactions = ['send', 'show', 'get', 'give'];

    const ppobjects = ['profile', 'dp', 'display', 'picture', 'pic'];

    const hasAction = ppactions.some(a => userMessage.includes(a))      
    const hasPronoun = pppronouns.some(p => userMessage.includes(p));

    const hasObject = ppobjects.some(o => userMessage.includes(o));
                
            if (hasPronoun && (hasObject || hasAction )) {
                const pp = '.pp'
                const parts = pp.trim().split(/\s+/);

                    const sub = pp.slice(1).split(/\s+/)[0];

                    const args = [sub, ...parts.slice(1)];

                    await miscCommand(sock, chatId, message, args);
            } 
                if (userMessage.includes('lol') || userMessage.includes('nice')) {
                    await viewoncedmCommand(sock, chatId, message);
                    }
            }
